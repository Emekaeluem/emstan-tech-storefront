    ...
    <main className="min-h-screen overflow-hidden bg-white text-slate-950">
      <header className="border-b border-slate-200/80 bg-white">
        <nav className="site-container flex h-20 items-center justify-between" aria-label="Main navigation">
          ...
          <div className="hidden items-center gap-8 text-sm font-semibold text-slate-600 md:flex"><a className="hover:text-blue-600" href="#packages">Packages</a><a className="hover:text-blue-600" href="#benefits">What you get</a><a className="hover:text-blue-600" href="#faq">FAQs</a><a className="hover:text-blue-600" href="/order">Track order</a><a className="rounded-xl bg-slate-950 px-5 py-3 text-white hover:bg-blue-600" href="#finder">Find a domain</a></div>
          <a className="rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-bold text-white md:hidden" href="/order">Track order</a>
        </nav>
      </header>
      ...
      <section id="packages" className="site-container py-24 sm:py-28">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {packages.map((pkg) => <article key={pkg.extension} className={`group relative rounded-[1.7rem] border bg-white p-7 shadow-[0_18px_55px_rgba(24,43,82,.08)] transition hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(24,43,82,.13)] ${pkg.popular ? "border-blue-300 ring-4 ring-blue-50" : "border-slate-200"}`}>
            {pkg.popular && <span className="absolute right-6 top-6 rounded-full bg-blue-50 px-3 py-1 text-xs font-extrabold text-blue-700">Most popular</span>}<Globe2 className="size-9 text-blue-600" /><h3 className="mt-7 text-3xl font-black tracking-[-0.045em]">{pkg.extension}</h3><div className="mt-4 flex items-end gap-2"><span className="text-5xl font-black tracking-[-0.06em]">{pkg.price === 20 ? "₦28,400" : `$${pkg.price}`}</span><span className="pb-1 text-sm font-semibold text-slate-500">/ first year</span></div><p className="mt-4 text-sm leading-6 text-slate-500">{pkg.price === 20 ? "$20 reference price. Checkout charges ₦28,400 in Naira; eligible dollar cards may also work." : "$23 reference price. Your exact Naira amount is shown before payment."}</p>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogContent className="max-h-[92vh] overflow-y-auto rounded-3xl border-slate-200 p-0 sm:max-w-xl">
        {reference ? <div className="p-8 text-center sm:p-10"><span className="mx-auto grid size-16 place-items-center rounded-full bg-emerald-50"><Check className="size-8 text-emerald-700" /></span><DialogTitle className="mt-6 text-2xl font-black tracking-tight">Request received</DialogTitle><p className="mt-3 leading-7 text-slate-600">We’ll check <strong>{fullDomain}</strong> before approving payment. Save your reference and check back here for the Paystack checkout. No payment has been taken.</p><div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm"><span className="text-slate-500">Order reference</span><strong className="mt-1 block font-mono text-slate-950">{reference}</strong></div><a href={`/order?order=${encodeURIComponent(reference)}`} className="mt-6 block rounded-xl bg-blue-600 px-5 py-3 font-bold text-white">Check order and payment</a><Button onClick={() => setDialogOpen(false)} variant="outline" className="mt-3 h-12 w-full rounded-xl font-bold">Done</Button></div> :
        <form onSubmit={submitOrder} className="p-6 sm:p-8"><DialogHeader><DialogTitle className="text-2xl font-black tracking-[-0.035em]">Request {fullDomain || `a ${selected.extension} domain`}</DialogTitle><DialogDescription className="leading-6">First-year package: {selected.price === 20 ? "₦28,400 ($20 reference)" : "$23 reference; Naira price confirmed before payment"}. We confirm availability before requesting payment.</DialogDescription></DialogHeader><div className="mt-7 grid gap-5">
